...
from agents import position_agent, signal_agent, industry_agent, advisor_agent

def load_transactions_from_db(db_path: str = "transactions.db", limit: int = 20) -> List[Dict[str, Any]]:
    """
    从数据库加载交易记录
        ...
        for row in rows:
            ...
            if not tx_dict.get('parsed_json'):
                try:
                    ...
                    # 简单解析
                    parsed_data = {
                        "action": "transfer" if int(tx_dict.get('value', '0')) > 0 else "unknown",
                        "token": "ETH",
                        "amount": str(int(tx_dict.get('value', '0')) / 10**18),
                        "time": datetime.fromtimestamp(int(tx_dict.get('time', '0'))).isoformat() + "Z",
                        "confidence": 0.5,
...
def run_analysis():
    """运行完整的分析流程"""
    print("=" * 60)
    print("开始执行多Agent分析工作流")
    print("=" * 60)
    
    # 检查环境变量
    if not os.getenv("CLAUDE_API_KEY"):
        print("警告: CLAUDE_API_KEY 环境变量未设置，将使用示例数据")
    ...
    # 执行工作流
    try:
        results = workflow.execute_workflow(transactions)
